...
from common.rl_lib import ReplayBuffer, OUNoise, ENoise, small_ValueNetwork, small_PolicyNetwork, ddpg_update, calc_values_of_states
...
value_net  = small_ValueNetwork(state_dim, action_dim, hidden_dim, device).to(device)
policy_net = small_PolicyNetwork(state_dim, action_dim, hidden_dim, device).to(device)

target_value_net  = small_ValueNetwork(state_dim, action_dim, hidden_dim, device).to(device)
target_policy_net = small_PolicyNetwork(state_dim, action_dim, hidden_dim, device).to(device)
...
# Make model
ddpg_net = small_PolicyNetwork(state_dim, action_dim, hidden_dim, device).to(device)
