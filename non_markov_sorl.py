...
# python ./non_markov_sorl.py --env=csense --gamma=0.997 --noise=0.7 --seed=123
...
################################################################################
# removes time and menergy obs
class non_markov_Wrapper(gym.ObservationWrapper):
    def __init__(self, env):
        super(non_markov_Wrapper, self).__init__(env)
        # Observation = [h_energy, p_energy, b_energy, req]
        self.observation_space = gym.spaces.Box(low=0, 
                                                high=1, 
                                                shape=(4,))
    def observation(self, obs):
        remove_index = [0,4]
        non_markov_obs = np.delete(obs, remove_index)
        return non_markov_obs

# In[6]:
################################################################################
# Setting up environment base
experiment = "non_markov_g"+str(GAMMA)+"-n"+str(max_noise)
